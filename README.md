...
- Trakt (İzliyorum sekmesinde): varsayılan olarak Trakt dışa aktarım ZIP'i yüklenir (VIP gerekmez). VIP varsa kullanıcının kendi açtığı Trakt API uygulamasıyla cihaz kodu akışı ve otomatik senkron; geçmiş, puan ve izleme listesi içe aktarılır. Client ID/Secret ve belirteçler yalnızca `chrome.storage.local`'da durur, JSON yedeğine ve buluta girmez.
