...
- **Trakt.** Trakt, API uygulaması açmayı Ağustos 2026'da VIP'e bağladığı için varsayılan yol ücretsiz dışa aktarım: Trakt'ta Ayarlar → Veri'den inen ZIP Perde'ye bırakılıyor. ZIP bağımlılıksız okunuyor (tarayıcının kendi DecompressionStream'i), dosyalar belgelenmemiş adlarına değil içeriklerine göre ayrılıyor. VIP'i olan için canlı bağlantı da duruyor: cihaz kodu akışıyla (trakt.tv/activate'te kısa kod; şifre eklentiye girmez) izleme geçmişi, puanlar ve izleme listesi içe aktarılıyor. Puan 7+ beğendim, 4 ve altı beğenmedim; izleme listesi Listem'e giriyor; Perde'de verilmiş yargı ezilmiyor. Perde'nin kendi yakaladığı aynı izleme (aynı bölüm, 12 saat içinde) ikinci kez sayılmıyor, tekrar senkron zararsız. Erişim belirteci 24 saatte bir kendiliğinden yenileniyor; bağlıyken Perde her açılışta 6 saatten eski senkronu arka planda tazeliyor. Trakt'tan gelen yıllar önceki diziler "Devam et" rafına (90 gün) ve "Yarıda kalanlar"a (180 gün) girmiyor.
